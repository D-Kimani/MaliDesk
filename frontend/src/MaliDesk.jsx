...
const storageSet = async (key, value) => {
  ...
  try { window.localStorage?.setItem(key, serialised); return true; } catch { return false; }
};
...
function MaliDeskCore({ auth }) {
  ...
  useEffect(() => {
    (async () => {
      let localData = await storageGet(DATA_KEY);
      if (!localData) {
        localData = await storageGet(LEGACY_DATA_KEY);
        if (localData) await storageSet(DATA_KEY, normaliseData(localData));
      }
      try {
        const remote = await authApi("/api/data");
        serverVersionRef.current = Number(remote.version);
        const remoteData = normaliseData(remote.data);
        const remoteHasData = remoteData.units.length || remoteData.tenants.length || remoteData.transactions.length;
        if (remoteHasData) {
          skipNextSaveRef.current = true;
          setData(remoteData);
        } else if (localData) {
          const local = normaliseData(localData);
          setData(local);
          if (auth.user.role === "Administrator") {
            const boot = await authApi("/api/data/bootstrap", { method:"POST", body:JSON.stringify({data:local}) });
            serverVersionRef.current = Number(boot.version);
          }
        } else {
          const initial = normaliseData(seedData());
          setData(initial);
          if (auth.user.role === "Administrator") {
            const boot = await authApi("/api/data/bootstrap", { method:"POST", body:JSON.stringify({data:initial}) });
            serverVersionRef.current = Number(boot.version);
          }
        }
      } catch {
        setData(normaliseData(localData || seedData()));
      } finally {
  ...
  useEffect(() => {
    if (!loaded || !data || !hydratedRef.current) return;
    if (skipNextSaveRef.current) { skipNextSaveRef.current = false; return; }
    storageSet(DATA_KEY, normaliseData(data)).catch(() => {});
    clearTimeout(saveTimerRef.current);
    saveTimerRef.current = setTimeout(async () => {
      if (serverVersionRef.current == null || syncBusyRef.current || !navigator.onLine) return;
      syncBusyRef.current = true;
      try {
        const result = await authApi("/api/data", { method:"PUT", body:JSON.stringify({ expectedVersion:serverVersionRef.current, data:normaliseData(data) }) });
        serverVersionRef.current = Number(result.version);
      } catch (err) {
        if (err.message?.includes("changed elsewhere")) {
          try {
            const remote = await authApi("/api/data");
            serverVersionRef.current = Number(remote.version);
            setData(normaliseData(remote.data));
            storageSet(DATA_KEY, normaliseData(remote.data)).catch(() => {});
            showToast("Another user changed the data. MaliDesk reloaded the latest version.");
          } catch {}
        } else if (!navigator.onLine) {
          // The local cache remains available and will be used while offline.
        }
    ...
    const sync = async () => {
      ...
      try {
        ...
        if (serverVersionRef.current == null) {
          serverVersionRef.current = Number(remote.version);
          if (!((remote.data?.units||[]).length || (remote.data?.tenants||[]).length || (remote.data?.transactions||[]).length)) {
            const local = await storageGet(DATA_KEY);
            if (local && auth.user.role === "Administrator") {
              const boot = await authApi("/api/data/bootstrap", {method:"POST",body:JSON.stringify({data:normaliseData(local)})});
              serverVersionRef.current = Number(boot.version);
            }
          }
        }
...
function ChangePasswordScreen({ auth, forced=false }) {
  const [currentPassword,setCurrentPassword]=useState("");
  const [newPassword,setNewPassword]=useState("");
  const [confirm,setConfirm]=useState("");
  const [busy,setBusy]=useState(false); const [error,setError]=useState("");
  const submit=async(e)=>{e.preventDefault();setError("");if(newPassword!==confirm)return setError("New passwords do not match.");setBusy(true);try{await authApi("/api/auth/change-password",{method:"POST",body:JSON.stringify({currentPassword,newPassword})});window.location.reload();}catch(err){setError(err.message)}finally{setBusy(false)}};
...
function UserManagement({ auth }) {
  const [users, setUsers] = useState([]); const [open, setOpen] = useState(false); const [busy, setBusy] = useState(false); const [error, setError] = useState(""); const [tempPassword,setTempPassword]=useState("");
  ...
  const resetPassword = async (u) => { setError(""); try { const r=await authApi(`/api/users/${u.id}/reset-password`,{method:"POST"}); setTempPassword(`${u.username}: ${r.temporaryPassword}`); } catch(e){setError(e.message)} };
...
export default function MaliDesk() {
  ...
  if (user.mustChangePassword) return <ChangePasswordScreen auth={{user,logout}} forced />;
